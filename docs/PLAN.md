...
### A.2. Что читает один токен (11.09.2026)

Тот же закон, доведённый до конца: делитель — не размер файла, а веса,
которые читает ОДИН токен. У плотной модели это все веса. У смеси экспертов
— общая часть плюс те эксперты, которых выбрал роутер. llama-bench b10826,
полная выгрузка на GPU, `-fa 1`:

| модель | активно на токен | prompt t/s | gen t/s |
|---|---|---|---|
| gpt-oss-20b MXFP4 (MoE, 4 из 32) | **3.19 ГБ** | **531** | **26.3** |
| Qwen3.8-27B UD-IQ4_XS (плотная) | 14.24 ГБ | 52 | 4.19 |
| Qwen3.8-27B Q4_K_M (плотная) | 16.80 ГБ | 53 | 4.29 |

Меньший файл (11.3 ГиБ против 13.3) работает в **семь раз** быстрее, потому
что читает в четыре с половиной раза меньше. На реальной глубине контекста
это держится: 4096 → 404 / 24.7, 16384 → 239 / 22.1.

Отсюда `@shared/models/speed.ts`: предсказание ≈ полоса × 0.75 / активные
байты, попадает в 20 % на всех трёх. И отсюда же — badge со скоростью в
списке моделей и `generation_tps` в `/monet-local/v1/models`: цифру, которая
решает, стоит ли грузить модель, клиент сам вычислить не может.

Двадцать-тридцать токенов в секунду на плотной 27B на этой машине
недостижимы ни при какой настройке: понадобилось бы 285–427 ГБ/с против
89.6 физических. Скорость даёт другая **архитектура**, а не другие флаги.

### A.3. Спекулятивное декодирование на 27B (11.09.2026)

Эксперимент «поменять математику»: один проход весов покупает не один
токен, а блок — черновик предлагает K токенов, 27B проверяет их за один
forward. На памяти-ограниченном железе это единственный способ обойти
деление «полоса ÷ веса». Qwen3.8-27B UD-IQ4_XS, 58/65 слоёв на GPU, 8K,
llama-server через `/completion`, промпт — задача на код, 96 токенов:

| вариант | gen t/s | принятие | ср. длина |
|---|---|---|---|
| KV в RAM, q8/q4 (профиль Авто до сегодня) | **0.70** | — | — |
| KV в RAM, f16 | 0.70 | — | — |
| KV на GPU, f16 — база | 3.58 | — | — |
| + ngram-mod | 3.67 | — | — |
| + DFlash2 Q4_K_M, n-max 7 (по умолчанию из README) | 4.09 | 71 % | 5.59 |
| + DFlash2, n-max 4 | 4.70 | 90 % | — |
| + DFlash2, n-max 3 | 6.66 (5.41 на 160 ток.) | 95 % (71 %) | 3.80 |
| + DFlash2, n-max 2 | 6.56 | 92 % | — |
| + draft-mtp (своя голова), n-max 3 | 5.80 | 82 % | 3.39 |
| + **draft-mtp, n-max 2** | **6.83** | 91 % | — |
| всё то же при 66/66 слоёв | 0xC0000005 | — | — |

Три вывода.

1. `--no-kv-offload` стоит **четыре пятых скорости**, а не «четверть»: всё
   внимание уходит на CPU против кэша, до которого GPU не дотягивается.
   Квантование кэша ни при чём (f16 в RAM — те же 0.70). Авто теперь
   выбирает размещение раньше размера контекста и отдаёт кэш в RAM
   последним перед CPU.
2. Спекуляция даёт **1.6–1.9×**, не 4–5×, как в статьях: на встроенной
   видеокарте проверка блока не бесплатна, поэтому короткие черновики (2–3)
   бьют длинные (7). Собственная MTP-голова модели не хуже отдельного
   драфтера DFlash2 и не требует файла. Авто включает `draft-mtp`,
   n-max 2 для любой модели с головой.
3. Потолок плотной 27B здесь — около 7 t/s. 20–30 на ней недостижимы:
   выше — только другая архитектура (см. A.2: MoE, 26 t/s).
